...
public class AttentionFirewallService extends AccessibilityService {
    ...
    private SystemPhysicsController systemPhysicsController;
    ...
    private SupportedBrowserConfig pendingBrowserAddressClear = null;
    ...
    private void handleGateCancelled() {
        ...
        // navigate to about:blank so the forbidden URL is gone.
        String interceptedApp = appPreferencesManager.getLastInterceptedApp();
        if (interceptedApp != null && !interceptedApp.isEmpty() && supportedBrowsers != null) {
            for (SupportedBrowserConfig config : supportedBrowsers) {
                if (interceptedApp.equals(config.packageName)) {
                    urlCheckHandler.postDelayed(() -> clearBrowserAddressBar(config), 500);
                    return;
                }
            }
        }
    ...
    protected void onServiceConnected() {
        ...
        systemPhysicsController = new SystemPhysicsController(this);
    ...
    private void onForegroundAppChanged(String packageName) {
        ...
        if (pendingBrowserAddressClear != null
            ...
            final SupportedBrowserConfig config = pendingBrowserAddressClear;
            ...
            urlCheckHandler.postDelayed(() -> {
                ...
                if (observed != null) {
                    boolean stillForbidden = false;
                    for (String pattern : appPreferencesManager.getForbiddenUrls()) {
                        String p = pattern.toLowerCase().trim();
                        if (!p.isEmpty() && observed.contains(p)) { stillForbidden = true; break; }
                    }
                    if (stillForbidden) clearBrowserAddressBar(config);
                }
            }, 1000);
        }
    ...
    private void checkBrowserUrl(SupportedBrowserConfig config, int eventType) {
        ...
        if (matchedPattern != null && committed) {
            ...
            // Apply cooldown whether or not a session is active. This prevents an
            // immediate re-gate when Chrome returns to the foreground after the user
            // cancels the gate (no active session) as well as mid-session gate loops.
    ...
    /**
     * Navigates the browser away from the forbidden URL by launching an Intent for
     * "about:blank" directly. This is reliable across all Chrome versions and does not
     * depend on accessibility tree manipulation of the URL bar.
     * Also pre-clears the observed URL cache so the next content event does not re-gate.
     */
    private void clearBrowserAddressBar(SupportedBrowserConfig config) {
        try {
            // Firefox's internal new-tab page is about:home; other browsers use about:blank.
            String safeUrl = config.packageName.contains("firefox") ? "about:home" : "about:blank";

            // Pre-clear the cached URL so a stale forbidden value does not immediately
            // re-trigger enforcement while Chrome is still processing the navigation.
            ...
            Log.e(TAG, "Failed to navigate browser away from forbidden URL", e);
            performGlobalAction(GLOBAL_ACTION_HOME);
        }
    }
    ...
    private void startStickySession(String packageName) {
        ...
        systemPhysicsController.setGrayscaleEnabled(true);
    ...
    private void endStickySession() {
        systemPhysicsController.setGrayscaleEnabled(false);
        updateForbiddenTimer(false);
    ...
    /** Launches the gate in exhausted mode: proceed button disabled, budget-depleted message shown. */
    private void triggerExhaustedGate() {
        pendingBrowserAddressClear = null;
        lastForegroundPackage = null;
        Intent intent = new Intent(this, DecisionGateActivity.class);
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        intent.putExtra(DecisionGateActivity.EXTRA_EXHAUSTED, true);
        startActivity(intent);
    }
    ...
    @Override public void onDestroy() {
        super.onDestroy();
        if (systemPhysicsController != null) {
            systemPhysicsController.setGrayscaleEnabled(false);
        }
        instance = null;
    }
